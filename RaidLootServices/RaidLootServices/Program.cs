...
namespace RaidLootBot
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            // --- 2. ПРОВЕРКА ФАЙЛОВ ---
            if (!File.Exists("settings.json") || !File.Exists("raids.json"))
            {
                Console.WriteLine("[ОШИБКА] Файлы settings.json или raids.json не найдены!");
                Console.WriteLine("Текущая папка: " + AppDomain.CurrentDomain.BaseDirectory);
                return;
            }

            // --- 3. ЗАГРУЗКА БАЗЫ ПРЕДМЕТОВ ---
            Console.WriteLine("\n📦 Загрузка базы предметов...");
            List<Item> loadingItems = new();

            if (!File.Exists("mangos_items.json"))
            {
                Console.WriteLine("⚠️ Файл mangos_items.json не найден.");
                Console.WriteLine("🔄 Создаём базу из Mangos...");

                var extractor = new MangosExtractor();

                if (await extractor.TestConnectionAsync())
                {
                    await extractor.SaveToJsonAsync("mangos_items.json");
                }
                else
                {
            ...
            Console.WriteLine("📥 Чтение mangos_items.json...");

            try
            {
                var json = await File.ReadAllTextAsync("mangos_items.json");
